...
export const DijkstraForm = () => {
  ...
  const countDijkstra = () => {
    toast.promise(findShortest(fromId, toId), {
      loading: "Вычисляем...",
      success: (response) => "Вычисленное значение: " + response.data,
      error: (err) =>
        get(err, "response.data.message", "Error, проверьте поля!"),
    });
  };

  return (
    <Form>
      <div className="optitle mt-2">
        Найти кратчайшее расстояние между двумя локациями по их ID
      </div>
